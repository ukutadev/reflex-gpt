...
def message_box(chat_message:ChatMessage) -> rx.Component:
    return rx.box(
        rx.box(
            rx.markdown(chat_message.message,
            background_color=rx.cond(chat_message.is_bot, rx.color("mauve",4),
                                      rx.color('blue',4)),
            color=rx.cond(chat_message.is_bot, rx.color("mauve",12),
            rx.color('blue',12)),
            **message_style,
        ),
        text_align=rx.cond(chat_message.is_bot,
         "left","right"),
        margin_top="1em",
    ),
    width="100%",
...
def chat_page():
    return ui.base_layout(
        rx.vstack(
            ...
            rx.box(
                #foreach, first part is the list/Iteration, second part is the function to render each item
